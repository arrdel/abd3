...
def test_sample_returns_tuple_lengths_match_flag():
    ...
    torch.manual_seed(0)
    out_3 = model.sample(n_samples=1, num_steps=3, track_nfe_per_block=True, progress=False)
    assert len(out_3) == 3
...
def test_sampled_tokens_stay_in_vocab_range():
    ...
    assert (
        x == model.mask_index
    ).sum().item() == 0, "mask_index leaked into final samples — substitution param broken?"
